...
class Video extends Component {
  ...
  connectToSocketServer = () => {
    ...
    socket.on('connect', () => {
      ...
      //socket.on('upvote', this.addupvote)
      ...
      //upvote
      // socket.on("upvote", (product, sender, room) => {
      //   // if (votes[room] == undefined) {
      //   //   votes[room] = { [product]: [sender] };
      //   // } else
      //   //   votes[room] = { [product]: new Set([...votes[room][product], sender]) };
      //   // io.sockets.emit("upvote", product, [...votes[room][product]].length);
      //   // console.log(votes);
      // });
  ...
  incrementMe = (ln) => {
    ...
    if(!this.state.ck[ln])
    {
    let currlikes = this.state.countMap[ln];
    if(currlikes) {
      currlikes++;
    } else {
      currlikes = 1;
    }
    this.state.countMap[ln] = currlikes;
    this.setState({});
...
//    socket.emit('upvote', this.state.countMap);
  ...
  // addupvote = (countMap) => {
  //   this.setState(prevState => ({
  //     messages: [...prevState.countMap, { 'count': this.state.count }],
  //   }))
  //   if (socketIdSender !== socketId) {
          ...
          <div>
            ...
            <Modal size="xl" show={this.state.showModal} onHide={this.closeChat} style={{ zIndex: '9999999' }}>
              ...
              <Modal.Body style={{ overflow: 'auto', overflowY: 'auto', height: '700px', textAlign: 'left' }}>
				...
				// 	</div>);
				// }
                  rows = [...rows, ...ans.map((ansi, i) => {
                    return <div key={i} style={{ textAlign: 'left' }}>
                      <p>
                        <iframe src={ansi} style={{ height: '60vh', width: '100%' }}/>
                      </p>
                      <button onClick={() => {
                      //  this.sendupvote;
                         this.incrementMe(ansi);
                      }}>❤️ Likes: {this.state.countMap[ansi]} </button>
                    </div>
                  })]
                  return (rows)
                }) : <p>No message yet</p>}
              ...
              <Modal.Footer size="lg" className="div-send-msg">
                <textarea style={{ height: '100px', width: '700px', padding: '5px' }} placeholder="Message"
